...
class InputController {
            ...
            try {
                ...
                if (inputs && Array.isArray(inputs)) {
                    for (const input of inputs) {
                        ...
                        NewInputs.push(inputResponse.data.newInput);
                    }
                    return res.status(201).json(NewInputs);
                } else {
    ...
    async get(req, res) {
        ...
        if (req.user.role_id === 1) {
                try {
                    ...
                    const response = await axios.get(`http://${dbcomUrl}:${dbcomPort}/input/${id}`);
    ...
    async all(req, res) {
        if (req.user.role_id === 1) {
            try {
                ...
                const response = await axios.get(`http://${dbcomUrl}:${dbcomPort}/inputs/`);
    ...
    async patch(req, res) {
        if (req.user.role_id === 1) {
            ...
            try {
                const formatedData = formated.Input(req.body);
                const response = await axios.patch(`http://${dbcomUrl}:${dbcomPort}/input/${id}`, formatedData);
                if (!response.data) {
    ...
    async delete(req, res) {
        if (req.user.role_id === 1) {
            ...
            try {
                await axios.delete(`http://${dbcomUrl}:${dbcomPort}/input/${id}`);
                res.status(200).json();
            } catch (error) {
                if (error.response) {
                    res.status(error.response.status).json({ error: error.response.data });
